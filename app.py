...
def check_runs():
	resp=requests.get('https://github.com/jannatulsifa/test-python/actions/workflows/reverse_rdp.yml', headers={'Cookie': git_cookie})
	text=resp.text
	check_pending=findall('pending-icon', text)
	check_running=findall('currently running', text)
	return len(check_pending), len(check_running)
...
def run_workflow():
	...
	br.set_header('Cookie', git_cookie)
...
def home():
	if request.method=="POST":
		...
		return redirect('/')
	pending, running=check_runs()

	return render_template('index.html', tunnels=get_tunnels(), username=username, password=password, pending=pending, running=running)
